...
public:
	DNS():seq_num(0){server_ips = getDNSServer(); s.DNS_sock_create();}
    ...
    string make_result_string(const char* buf, int offset);
	...
	int send_DNS_request(string hostname);
    pair<int,string> DNS::get_DNS_answer();
	unsigned long get_seq(){return seq_num;} 
};
